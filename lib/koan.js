...
ZenLocation.prototype.request = function(options, callback) {
    ...
    return https.request(options, function(response) {
        ...
        response.on('end', function() {
            callback(null, requester.enrich(JSON.parse(received)));
        });
...
ZenLocation.prototype.post = function(data, callback) {
    if(!callback) {
        callback = data;
        data = {};
    }
...
ZenLocation.prototype.get = function(data, callback) {
    if(!callback) {
        callback = data;
        data = {};
    }
...
Job.prototype.enrich = function(data) {
    for(var index = 0, len = data.outputs.length; index < len; ++index) {
        ...
        data.outputs[index] = transfer(
            new Output(this, rawOutput.id + '/progress'),
            rawOutput
        );
    }
    return data;
